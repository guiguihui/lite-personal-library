...
(function () {
  ...
  // H3: 把 OpenAI 风格的 messages 转换为 Anthropic /v1/messages 格式。
  // OpenAI: {role:'assistant', tool_calls:[{id, function:{name, arguments}}]}
  //         {role:'tool', tool_call_id, content}
  // Anthropic: assistant 消息用 content blocks(tool_use),tool 结果用
  //           {role:'user', content:[{type:'tool_result', tool_use_id, content}]}
  function convertMessagesForAnthropic(messages) {
    var out = [];
    for (var i = 0; i < messages.length; i++) {
      var m = messages[i];
      // L9: 剥离非标准字段(如 citations),避免发给上游
      var content = m.content;
      var role = m.role;

      if (role === 'tool') {
        // OpenAI tool 结果 → Anthropic user + tool_result block
        out.push({
          role: 'user',
          content: [{ type: 'tool_result', tool_use_id: m.tool_call_id || '', content: content || '' }]
        });
        continue;
      }

      if (role === 'assistant' && m.tool_calls && m.tool_calls.length) {
      ...
      // 普通消息:剥掉 citations 等多余字段
      var clean = { role: role, content: content || '' };
      out.push(clean);
    }
    return out;
  }
  ...
  function buildRequest(opts) {
    ...
    if (proto === 'anthropic') {
      // H3: 把 OpenAI 风格 messages(tool_calls/tool 角色)转为 Anthropic 格式
      var anthropicMessages = convertMessagesForAnthropic(messages);
      return {
        url: url,
        headers: {
