...
(function () {
  ...
  async function executeTool(name, args, budgetCtx, sourceCtx) {
    if (name === 'search_library') {
      var r = await retrieveContextAsText(args.query, budgetCtx, sourceCtx);
  ...
  async function sendMessage(query, refs) {
    ...
    try {
      ...
      for (var loop = 0; loop < MAX_LOOPS; loop++) {
        ...
        if (stopReason !== 'tool_calls' || !toolCalls || !toolCalls.length) break;
      ...
      contentEl.innerHTML += '<br><span style="color:#dc2626">错误: ' + window.LqdChatCitations.escHtml(e.message) + '</span>';
    }
