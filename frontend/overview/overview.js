...
(function () {
  ...
  // type 单数(book/paper/note) → 复数(books/papers/notes),library/openDoc 用复数
  var TYPE_TO_PLURAL = { book: 'books', paper: 'papers', note: 'notes' };
  function normalizeTypePlural(t) {
    if (!t) return '';
    return TYPE_TO_PLURAL[t] || t;
  }

  // 解析 sourceId(type:doc_id:node_id);doc_id/node_id 可能含冒号,按首、尾两段切
  function parseSourceId(sourceId) {
    ...
    out.type = sourceId.slice(0, first);
    if (last > first) {
  ...
  function renderChatOverview(container) {
    ...
    for (var i = 0; i < state.chatContexts.length; i++) {
      ...
      item.addEventListener('click', (function (ctx) {
        return function () {
          // context 无 docId 字段,从 sourceId(type:doc_id:node_id)解析。
          // type 归一化为复数(library/openDoc 用复数,与 /api/search 一致)。
          var parsed = parseSourceId(ctx.sourceId);
          var docType = normalizeTypePlural(ctx.docType || parsed.type || '');
          var slug = ctx.docId || parsed.docId || '';
          var nodeId = ctx.nodeId || parsed.nodeId || '';
          if (window.LqdEvents) {
          ...
          if (slug && window.LqdLibrary && typeof window.LqdLibrary.openDoc === 'function') {
            window.LqdLibrary.openDoc(docType, slug, nodeId);
          }
        };
      })(c));
  ...
  function renderLibraryOverview(container, tab) {
    ...
    // P2 知识链接:总览底部渲染当前文档的局部图谱(一跳邻居)。
    // docId 取 tab.state(含 doc 元信息)中的 type/slug,归一化为单数 + ':' + slug。
