...
(function () {
  ...
  function registerDefaultCommands() {
    ...
    register({
      ...
      icon: 'refresh',
      action: function () {
        // M4 修复:manage 组件对外暴露的命名空间是 YuuManage(见 manage/index.js),
        // 此前命令面板错误引用 LqdManage.buildIndex 导致命令为"死命令"。
        // 优先 YuuManage,回退 LqdManage 以保持向前兼容。
        var mgr = window.YuuManage || window.LqdManage;
        if (mgr && typeof mgr.buildIndex === 'function') {
          mgr.buildIndex('incremental');
        } else if (window.console && window.console.warn) {
          window.console.warn('[LqdCommands] index:build — manage 模块未就绪');
        }
      }
    });
