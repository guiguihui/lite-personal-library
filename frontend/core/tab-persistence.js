...
(function () {
  ...
  /**
   * M5 修复:tab.state 瘦身。
   * 此前 sanitizeState 仅过滤函数和下划线字段,但 library 标签会把
   * 整个 doc 对象(含正文、树结构、元数据)塞进 state.doc,
   * 多个 library 标签累积后 JSON 序列化体积可达数 MB,撑爆 localStorage(5MB 上限)。
   *
   * 策略:对已知大字段(doc/content/tree/sections)只保留轻量摘要
   * (type/slug/title),其余标量字段原样保留。
   */
  var HEAVY_KEYS = { doc: true, content: true, tree: true, sections: true, _currentDoc: true };
  var MAX_STATE_JSON_LEN = 80000; // 单标签 state 序列化上限 ~80KB
  ...
  function sanitizeState(state) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (!isSerializable(state[k])) continue;
      var v = shrinkValue(k, state[k]);
      // 二次校验:序列化后过长则丢弃
      try {
        if (JSON.stringify(v).length > MAX_STATE_JSON_LEN) continue;
      } catch (e) { continue; }
      clean[k] = v;
    }
    return clean;
