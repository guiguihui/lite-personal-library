...
def _stable_identity(value: os.stat_result) -> tuple[int, ...]:
    """Cross-platform stable identity for anti-drift checks.

    Same rationale as ``app.index.v3.supervisor._identity``: on Windows/NTFS,
    ``st_ctime_ns`` can be refreshed by opening a just-written file for read,
    so comparing it yields false "changed while reading" failures. Omit ctime
    on Windows; keep it elsewhere.
    """
...
def _hash_file(path: Path, field: str) -> tuple[str, int]:
    ...
    if _stable_identity(before) != _stable_identity(after) or observed != before.st_size:
...
def run_worker(request_path: Path) -> int:
    ...
    try:
        ...
        if len(payload) > MAX_JSON_LINE_BYTES:
            raise ProtocolError("request exceeds JSON-line byte limit")
        request_identity = _stable_identity
        if (
