...
def _result_from_dict(
    ...
    *,
    ...
    if (
        ...
        raise AuditProtocolError("result audit_pid must be a positive integer")
    if os.name != "nt" and audit_pid != expected_pid:
        # On POSIX, process.pid is the child's own pid (single exec), so strict
        # matching is safe. On Windows, a venv ``python.exe`` is a redirector
        # launcher: ``subprocess.Popen`` returns the launcher's pid while the
        # child reports the real interpreter's ``os.getpid()``, so the two
        # never match. We keep request_id as the strong correlation token and
        # skip the pid equality check on Windows.
