...
class Hit:
    ...
    - V3 稳定引用字段（search_view 注入，legacy 路径为空）：generation / view_id /
      doc_key / doc_uid / segment_hash / local_id / node_key。
    """
...
def _per_doc_truncate(scored: list[Hit], top_k: int) -> list[Hit]:
    """per-doc 截断：每文档最多 3 条，topK*2 break，slice topK。

    与 retrieval.js ``search`` / ``search_inverted`` / ``search_multi_path`` 一致。
    per-doc key 用稳定引用(doc_key/doc_uid)优先,回退 doc_id——
    这样同 slug 跨 doc_type(如 paper:alpha vs note:alpha)不会被合并。
    """
