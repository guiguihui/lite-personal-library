...
def rrf_fuse(paths: list[list[Hit] | None], k: int = 60) -> list[Hit]:
    ...
    """
    scores: dict[tuple, dict[str, Hit | float]] = {}
    for hits in paths:
        if not hits:
            continue
        for i, h in enumerate(hits):
            # 稳定引用优先:doc_uid + node_key 唯一标识文档节点(跨 doc_type 不合并);
            # 回退 legacy:doc_id + node_id(与 retrieval.js 对齐)。
            key = (
    ...
    for e in scores.values():
        ...
        # 注意:保留 V3 稳定引用字段(generation/view_id/doc_key/doc_uid/
        # segment_hash/local_id/node_key),供 search_view 溯源使用。
        fused = Hit(
            node=item.node,
