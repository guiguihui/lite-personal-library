...
def process_note(slug: str, note_path: str, source_prefix: str = "notes") -> tuple[dict | None, list[dict], list[dict]]:
    """Build PageIndex tree for a standalone note .md file.
    Returns (doc_tree, flat_nodes_for_index, chunk_nodes_with_text).

    source_prefix: 源目录名(默认 notes),使 source_md 指向 content/<source_prefix>/<slug>.md。
    """
    ...
    note_source_md = f"{RAW_PATH_PREFIX}/{source_prefix}/{slug}.md"
...
def write_inverted_index(all_chunk_nodes: list[dict], global_docs: list[dict]) -> None:
    ...
    stop_cap = int(n_chunks * STOPWORD_DF_RATIO) if n_chunks > 1 else "N/A"
    ...
    print(f"  chunks:       {n_chunks} chunks, "
          f"{kept_tokens} tokens (dropped {dropped_tokens} stopwords, cap={stop_cap})")
