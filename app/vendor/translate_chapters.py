...
async def _translate_once(client, body: str, glossary: dict, is_seed: bool, feedback: str = "", context_note: str = "") -> str:
    ...
    client 参数保留以兼容调用签名,实际忽略——改用 app.llm.nonstream.call_llm_once
    (复用 build_request 的双协议适配,支持 Anthropic 端点)。
    """
...
async def translate_book(book_dir: str, concurrency: int, seed_count: int, max_retry: int,
    ...
    # client 不再需要——_translate_once 改用 call_llm_once(双协议适配)。
    # translate_chapter 的 client 参数保留 None(向后兼容签名)。
