...
def delete_content_dir(doc_type: str, slug: str, cfg: AppConfig) -> bool:
    """删除 content/{type}s/{slug}/ 目录(含所有 md 文件)。

    返回 True 表示至少删除了一个文件,False 表示目录不存在。
    """
    import shutil

    dir_path = Path(cfg.content_dir) / f"{doc_type}s" / slug
...
def delete_pdf_dir(slug: str, cfg: AppConfig) -> bool:
    """删除 pdfs/{slug}/ 目录(如果存在)。"""
    import shutil

    dir_path = Path(cfg.pdfs_dir) / slug
    if not dir_path.is_dir():
        return False
    shutil.rmtree(dir_path)
    return True
