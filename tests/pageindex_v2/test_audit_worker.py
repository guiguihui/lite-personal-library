...
def test_deep_audit_runs_in_a_distinct_short_lived_process(tmp_path: Path) -> None:
    ...
    if os.name != "nt":
        # POSIX: single exec, Popen.pid == child os.getpid().
        assert result.audit_pid == handle.pid
    else:
        # Windows: venv python.exe is a redirector launcher, so Popen.pid is
        # the launcher while the child reports the real interpreter's pid.
        # Only require that the audit truly ran in another process.
        assert result.audit_pid != os.getpid()
